...
Pakiety frameworków (React, Vue itd.) są **opcjonalnymi** zależnościami peer — instaluj tylko ten, którego używasz.
...
**CDN `<script>` (bez bundlera)** — udostępnia `window.Scorm`:
```html
